...
function BreathingExercise() {
      ...
      <div className="breathing">
        ...
        <div className="breathing__instruction">
          ...
          <span style={{ color: 'var(--text-muted)', fontSize: 14 }}>
            4 секунды вдох — 4 задержка — 4 выдох
          </span>
        </div>
        <button className="btn btn--primary btn--lg" onClick={() => { setActive(true); setCycles(0); }}>
          Начать
        </button>
      </div>
    );
  ...
  return (
    <div className="breathing">
      ...
      <button className="btn btn--secondary" onClick={() => { setActive(false); setPhase('inhale'); setCounter(4); }}>
        Стоп
      </button>
    </div>
  );
  ...
  return (
    <div>
      <div className="focus-stat-grid mb-20">
        ...
        <div className="focus-stat">
          <div className="focus-stat__value">{weekHours}ч</div>
        ...
        <div className="focus-stat">
          <div className="focus-stat__value" style={{ color: pct >= 70 ? 'var(--success)' : 'var(--warning)' }}>
            {pct}%
          </div>
      ...
      {completed.length === 0 && cancelled.length === 0 && (
        <div className="empty-state">
          ...
          <div className="empty-state__text">Пока нет сессий.<br />Запустите свою первую!</div>
        </div>
      )}
      ...
      {completed.slice(-10).reverse().map((s) => {
        ...
        return (
          <div key={s.id} className="history-item">
            <div className="history-item__mood">
              {s.type === 'pomodoro' ? '🍅' : '🧠'}
            </div>
      ...
      <div className="focus-timer">
        <div className="tabs" style={{ width: '100%' }}>
          <button className={`tabs__item${type === 'pomodoro' ? ' tabs__item--active' : ''}`}
            onClick={() => { setType('pomodoro'); setMinutes(25); }}>
            🍅 Помодоро
          </button>
          <button className={`tabs__item${type === 'deepwork' ? ' tabs__item--active' : ''}`}
            onClick={() => { setType('deepwork'); setMinutes(60); }}>
            🧠 Глубокая работа
          </button>
        </div>
        ...
        <button className="btn btn--primary btn--lg" onClick={() => start(type, type === 'pomodoro' ? 25 : minutes)}>
          Начать
        </button>
      </div>
    );
...
export function FocusPage() {
  ...
  return (
    <div>
      ...
      <div className="tabs">
        <button className={`tabs__item${tab === 'timer' ? ' tabs__item--active' : ''}`}
          onClick={() => setTab('timer')}>⏱ Таймер</button>
        <button className={`tabs__item${tab === 'breathing' ? ' tabs__item--active' : ''}`}
          onClick={() => setTab('breathing')}>🫁 Дыхание</button>
        <button className={`tabs__item${tab === 'stats' ? ' tabs__item--active' : ''}`}
          onClick={() => setTab('stats')}>📊 Стат</button>
      </div>
      ...
      {/* Floating timer when on other tabs */}
      {tab !== 'timer' && current && current.status === 'running' && (
        <div style={{
          position: 'fixed', top: 12, right: 12,
          background: 'var(--accent)', color: 'white', borderRadius: 'var(--radius)',
          padding: '8px 16px', fontSize: 16, fontWeight: 700, fontVariantNumeric: 'tabular-nums',
          boxShadow: 'var(--shadow-glow)', zIndex: 50, cursor: 'pointer',
